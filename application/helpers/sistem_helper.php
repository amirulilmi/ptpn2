...
{
    $ci = &get_instance();
    $q = $ci->db->query("select * from user where username='$id'")->row_array();
    return $q['nama'];
}
function getfoto($id)
{
...
function gettotalalat()
{
    date_default_timezone_set('Asia/Jakarta');
    $currentTime = date('Y-m-d');

    $ci = &get_instance();
    $q = $ci->db->query("SELECT * FROM laporan_kerja_alat_berat WHERE date = ?", array($currentTime))->num_rows();

    // SELECT * FROM alat_berat WHERE date = :dateToSearch
    return $q;
}

function gettotalkebun()
{
    $ci = &get_instance();
    $q = $ci->db->query("select * from kebun ")->num_rows();
    return $q;
}

function getjumalat($tahun_perolehan)
{
    date_default_timezone_set('Asia/Jakarta');
    $currentTime = date('Y-m-d');
    ...
    $query = "SELECT * FROM laporan_kerja_alat_berat WHERE id_kebun = '$tahun_perolehan' AND date = ?";
    $result = $ci->db->query($query, array($currentTime));

    // Dapatkan jumlah baris yang cocok dengan kueri
    $q = $result->num_rows();


    // $q = $ci->db->query("select * from laporan_kerja_alat_berat where id_kebun='$tahun_perolehan'")->num_rows();
